...
const fetchProducts = async () => {
  try {
    const response = await fetch('http://localhost/distributorHub/showDistributorHompageProducts.php'); // Replace with your actual API URL
...
const selectProduct=(selectedProduct)=>{
  ...
  route.push('/distributor/product/page');
}
    ...
    <div class="products-cont">
      ...
      <div v-for="product in products" :key="product.prodId" class="products-box" v-on:click="selectProduct(product)">
        ...
        <div class="products-name">
          ...
          <p class="sub-title"><strong>Distributor's Name: </strong>{{ product.disUserName }}</p>
          <p class="sub-title">{{ product.prodRegion }}</p>
        </div>
      </div>
    </div>
